...
def interstory_drift_ratio(H, phi, G, D, N, t, n_modes = 5):
    ...
    # IDR = np.zeros((N,n_modes)) 
    
    # fig, (ax) = plt.subplots(dpi=300)
    # fig.set_size_inches(25,5)
    # ax.set(xlim=[0,35],ylim=[-1/100,1/100])
    
    # for md in range(n_modes):
        
    #     IDRmd = IDRr[:,md][:,None] * D[md,:]
    #     IDR[:,md] = np.amax(np.abs(IDRmd), axis=1)
        
    #     for j in range(N-1,N):
    #         plt.plot(t, IDRmd[j,:])
    #         print(np.amax(np.abs(IDRmd[j,:])))
        
    
    IDR = np.zeros((N,np.shape(D)[1])) 
    for i in range(N):
        IDRi = np.zeros((np.shape(D)[1]))
        for md in range(n_modes):
            IDRi += (IDRr[i,md] * D[md,:])**2
        IDR[i,:] = np.sqrt(IDRi)
...
# Stiffness matrix
k = 31.54
K = assemble_stiffness(k*np.ones(N))
